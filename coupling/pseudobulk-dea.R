...
matrices = Libra::to_pseudobulk(expr, meta = meta) %T>%
  saveRDS(glue("{outdir}/pseudobulk.input.rds"))
...
results <- bind_rows(
  ...
  run_de(input = expr, meta = meta, n_threads = 20, 
         de_family = "singlecell", de_method = "MAST"),
  run_de(input = expr, meta = meta, n_threads = 20, 
         de_family = "singlecell", de_method = "wilcox"),
  # run_de(input = expr, meta = meta, n_threads = 20, 
  #        de_family = "singlecell", de_method = "t"),
  # run_de(input = expr, meta = meta, n_threads = 20, 
  #        de_family = "singlecell", de_method = "LR")
) %>% rename(transcript_id = gene) %>% left_join(transToGenes, by = "transcript_id") %T>%
 readr::write_csv(glue("{outdir}/dea_all.info.csv"))
...
results %>% filter(p_val_adj < 0.05, abs(avg_logFC) > log(2)) %>% 
  ...
  readr::write_csv(glue("{outdir}/dea_all.stat.csv"))
...
DEAnalysis <- function(input, meta, group1, group2){
  ...
  results <- bind_rows(
    ...
    readr::write_csv(glue("{outdir}/{comp_groups}.dea.info.csv"))

  results %>% filter(p_val_adj < 0.05, abs(avg_logFC) > log(2)) %>%
    group_by(cell_type, de_method, de_type) %>%
    summarise(nTrans=n(), nGenes = length(unique(.data[["gene_id"]])) )  %T>%
    readr::write_csv(glue("{outdir}/{comp_groups}.dea.stat.csv"))
}
