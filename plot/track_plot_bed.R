...
transCounts <- read.table(argv$count, sep="\t", header = T, row.names = 1)
trans <- transcripts(transInfo, columns=c("tx_name"), use.names=FALSE)@elementMetadata$tx_name

for (sample in colnames(transCounts)){
  input1.df = c()
  input2.df = c()
  i = 1
  for (tran in rev(trans)){
    ...
    nTrans = transCounts[tran, sample]
    if (is.na(nTrans) || nTrans == 0) next
  ...
  write.table(input1.df %>% rename(transcript=gene, count=score) %>% distinct(transcript, count), 
              glue("{outdir}/{prefix}.{sample}.tsv"), 
              row.names = F, col.names = T, quote = F, sep="\t")
  # plot transcript structure
  pdf(glue("{outdir}/{prefix}.{sample}.pdf"), height = 12, width = 9)
