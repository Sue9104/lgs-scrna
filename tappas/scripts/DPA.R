...
library(argparser)
p <- arg_parser("Differential PolyAdenylation Analysis")
...
p <- add_argument(p, "--length", help="min length", default=60)
argv <- parse_args(p)
...
infoDPA = read.table(file.path(indir, "dpa_info.tsv"), sep="\t", quote=NULL, header=TRUE,  stringsAsFactors=FALSE)
...
for(gene in sort(unique(df.infoDPA$Gene))){
    print(gene)
    print(infoDPA[infoDPA$Gene==gene,]$Strand == "+")
...
write.table(a, file.path(outdir, "diff_PolyA_matrix_mean.tsv"), quote=FALSE, row.names=TRUE,col.names = FALSE, sep="\t")
