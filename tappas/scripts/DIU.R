...
library(argparser)
p <- arg_parser("Differential Isoform Usage Analysis")
...
p <- add_argument(p, "--method", help="DEXSEQ or EDGER", default='DEXSEQ')
...
result <- read.table(file.path(outdir, paste("result_", dataType, ".tsv", sep="")), sep="\t", header=T)
print(head(result))
genesMatrix = read.table(file.path(indir, "gene_matrix.tsv"), row.names=1, sep="\t", header=TRUE)
meanexp1 <- rowMeans(genesMatrix[,row.names(myfactors)[myfactors==1]])
meanexp2 <- rowMeans(genesMatrix[,row.names(myfactors)[myfactors==2]])
meanexp <- merge(meanexp1, meanexp2, by=0)
colnames(meanexp) <- c("gene", "MeanExpression1", "MeanExpression2")
print(head(meanexp))
library(tidyverse)
diu <- result %>% left_join(meanexp, by='gene') %>% rename(Gene=gene) %>%
    mutate(DS = if_else(qValue<0.05,'DIU','Not DIU'))  %>%
    mutate(podiumChange = if_else(podiumChange, 'YES', 'NO'))  %>%
    mutate(across(where(is.numeric), ~ round(.x, 3)))  %>%
    select(Gene,totalChange,podiumChange,DS,MeanExpression1,MeanExpression2)
print(head(diu))
write.table(diu, file.path(outdir, paste(dataType, "_diu_matrix.tsv", sep="")), quote=FALSE, row.names=FALSE, sep="\t")
